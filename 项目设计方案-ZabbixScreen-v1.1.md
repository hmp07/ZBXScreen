...
> **状态：已确认，可进入开发** | 确认日期：2026-06-01 | 文档版本：v1.2
>
> 本文档面向开发者，包含完整的架构设计、模块说明、接口规范、开发规范及环境搭建指南，可直接作为开发依据。
>
> **v1.2 更新说明**（2026-06-01）：
> - 修正 Dockerfile 中非 root 用户运行 nginx 的权限配置问题，增加 `docker-entrypoint.sh` 数据库初始化入口
> - 修正调度器架构：APScheduler 从 uvicorn worker 剥离为独立进程，避免多 worker 重复执行
> - 修正网络接口自动发现策略（适配 Predictable Network Interface Names）和磁盘多挂载点处理
> - 完善 AES-128-CBC 密码加密方案（增加随机 IV 策略和实现代码）
> - 新增 Webhook 告警恢复事件 Schema（`event_type: "recovery"`）和告警风暴抑制策略
> - 新增测试策略、数据库迁移（Alembic）、日志规范、规模边界与性能考量章节
> - 新增 Zabbix 5.x/6.x/7.x 版本兼容详情和大屏异常状态展示规范
> - 新增 `monitor_cache` 聚合缓存表设计，完善 API 分页规范
...
ZabbixScreen 是一款**专注于 Zabbix 监控数据可视化大屏展示**的轻量级系统。与 ITOps-Watch 相比，本项目做**减法**——去掉 Prometheus 多数据源、AI 巡检、网络拓扑、Web 监控等复杂功能，将精力集中在：

- **实时监控大屏**：自动汇聚 Zabbix 主机数据，提供开箱即用的默认大屏
- **主机历史数据趋势**：直观查看各主机历史监控数据
- **数据源管理**：多 Zabbix 实例统一接入与管理
- **Webhook 告警出口**：标准化告警数据推送，对接 AI 分析引擎进行智能研判
...
| **大屏聚焦** | 默认大屏 + 自定义大屏双模式，适配运维指挥中心场景 |
| **快速部署** | Docker Compose 一键启动，5 分钟上线，同时支持 amd64 和 ARM64 |
| **兼容性强** | 支持 Zabbix 5.x / 6.x / 7.x 版本 |
| **AI 就绪** | Webhook 告警推送采用标准化 JSON Schema，为对接 AI 分析引擎预留接口 |

### 1.3 与 ITOps-Watch 功能对比

| 功能模块 | ITOps-Watch | ZabbixScreen | 说明 |
|----------|-------------|--------------|------|
| Zabbix 数据源 | ✅ | ✅ | 核心功能 |
| Prometheus 数据源 | ✅ | ❌ | 精简，仅保留 Zabbix |
| 默认监控大屏 | ✅ | ✅ P1 | 保留，优化展示 |
| 自定义大屏编辑器 | ✅ | ✅ P2 | 延至二期 |
| 大屏轮播 | ✅ | ✅ P2 | 延至二期 |
| 主机历史数据 | ✅ | ✅ P1 | 保留 |
| 主机配置管理 | ✅ | ❌ | 精简，不管理 Zabbix 主机 |
| AI 智能巡检 | ✅ | ❌ | 精简（由外部 AI 引擎承接） |
| AI 智能告警 | ✅ | ❌ | 精简（由外部 AI 引擎承接） |
| Webhook 告警（AI引擎对接） | ✅ | ✅ P1 | **P1 必须实现**，含标准化 Schema |
| 多渠道通知（微信/钉钉/飞书） | ✅ | ❌ | 精简，仅保留 Webhook |
| Web 站点监控 | ✅ | ❌ | 精简 |
| 网络拓扑 | ✅ | ❌ | 精简 |
| 网络扫描 | ✅ | ❌ | 精简 |
| 导入导出 | ✅ | ✅ P2 | 自定义大屏导入导出，延至二期 |
| 多语言 | ✅ | ❌ | 精简，仅中文 |
| 地图告警 | ✅ | ✅ P3 | 延至三期 |

> P1 = 首期必做，P2 = 二期，P3 = 三期可选
...
| 阶段 | 模块 | 主要内容 | 优先级 |
|------|------|---------|--------|
| P1-1 | 脚手架 | 项目初始化（前端 Vue3+Vite、后端 FastAPI、Docker multi-arch 配置） | 🔴 必须 |
| P1-2 | 数据源管理 | 添加/测试/连接/启用禁用 Zabbix 数据源，密码加密存储 | 🔴 必须 |
| P1-3 | 默认监控大屏 | 主机状态卡片矩阵 + 统计卡片 + TOP N 图表 + 自动刷新 | 🔴 必须 |
| P1-4 | 主机监控 | 主机列表 + 主机详情 + 历史趋势图（多时间范围） | 🔴 必须 |
| P1-5 | 用户认证 | JWT 登录/刷新/修改密码，前端路由守卫 | 🔴 必须 |
| P1-6 | 告警模块 | 告警规则配置 + 告警记录展示 + 告警统计 | 🔴 必须 |
| P1-7 | Webhook 模块 | 端点配置 + 标准化推送（含 context） + 重试机制 + 推送日志 | 🔴 必须 |
| P1-8 | 系统设置 | 系统标题/刷新频率/数据保留天数/时区配置 | 🟡 重要 |
| P1-9 | 构建部署 | Docker multi-arch 镜像构建、docker-compose 生产/开发分离 | 🔴 必须 |

**P1 验收标准**：
- 可连接 Zabbix 5.x/6.x/7.x 并展示真实数据
- 默认大屏正常渲染，自动刷新无报错
- 告警产生时 Webhook 成功推送（含 context 字段）
- docker-compose up -d 一键启动，amd64 和 arm64 均可正常运行
